...
from nodi_libs.backoff import Backoff, BackoffStrategy, create_backoff
...
class MqttClient:
    ...
    def _supervisor_loop(self) -> None:
        # Track if we ever had a successful connection (for reconnect vs connect decision)
        ...
        while self._running and self._supervisor_enabled:
            ...
            try:
                if had_connection:
                    # Had connection before - use reconnect (reuses existing loop)
                    self._mqtt_client.reconnect()
                else:
                    # Never connected - use full connect + loop_start
    ...
    def start(self,
        ...
        # CRITICAL: Always start supervisor if enabled, even if initial connection fails
        # This ensures reconnection attempts continue when network becomes available
